...
class QuotaTimelineStatsTest(test.APITransactionTestCase):
    ...
    def test_timeline_formatted_as_list_of_dictionaries(self):
        factories.CloudProjectMembershipFactory(project=self.project)
        expected = [
            {
                'from': 1433808000,
                'to': 1433894399,
    ...
    def test_zabbix_is_called_with_right_parameters(self):
        expected_memberships = [factories.CloudProjectMembershipFactory(project=self.project) for _ in range(3)]
        # other memberships:
        [factories.CloudProjectMembershipFactory() for _ in range(3)]
        ...
        with patch('nodeconductor.monitoring.zabbix.db_client.ZabbixDBClient.get_projects_quota_timeline') as obj:
            ...
            obj.assert_called_once_with(
                [m.tenant_id for m in expected_memberships],
                ['project_{}_limit'.format(query_params['item']), 'project_{}_usage'.format(query_params['item'])],
                query_params['from'],
                query_params['to'],
